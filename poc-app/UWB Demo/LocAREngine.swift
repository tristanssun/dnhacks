...
final class LocAREngine {
    ...
    /// The peer's bearing to us — the azimuth of peer→us in the peer's own
    /// ARKit frame — applied to every particle.
    ///
    /// Both devices are gravity-aligned, so their frames differ by yaw alone:
    /// that is θ, and it is the only unknown in their relative orientation. A
    /// particle carries a position for the peer and a θ, so it predicts what
    /// the peer should have measured: rotate the us − peer vector into the
    /// peer's frame by −θ and take its azimuth. Particles whose prediction
    /// misses lose weight, whichever of position or θ is at fault.
    ///
    /// This used to be paired with a bearing of our own and solved outright,
    /// θ = ψ_us − ψ_them + π. That needs both phones bearing each other inside
    /// a lag window, and with one camera-assisted NISession per device the
    /// unassisted side rarely has one, so the exchange mostly skipped. Applied
    /// per particle the peer's bearing carries the same information on its
    /// own. Once `ingestBearing` has narrowed position to a ray only θ is left
    /// to explain the measurement, and that is the paired solve. Without our
    /// bearing it still cuts the joint (position, θ) cloud to the manifold
    /// θ = φ − ψ_them + π, φ being the particle's azimuth from us, and range
    /// with our own motion resolve φ over time. Either way θ is measured
    /// without the peer having to move, which no other θ source here manages:
    /// the rotation term in `ingestRemoteVIO` only discriminates in proportion
    /// to peer displacement, and `ingestRelativeEstimate` rotates an incoming
    /// vector *by* θ and so consumes it rather than measuring it.
    ///
    /// Independent bearings average as σ/√N, so ~7° from one becomes ~2° from
    /// ten — against a compass floor of tens of degrees.
    func ingestPeerBearing(peerID: MCPeerID, azimuth psi: Float) {
        guard hasLocal, let slot = slots[peerID], psi.isFinite else { return }
        let limit = 3 * sigmaBearing
        let inlierProbability = 1 - nlosProbability

        withCloud(at: slot) { cloud in
            cloud.withMutableBuffers { x, y, z, theta, weight, sine, cosine in
                var count = Int32(theta.count)
                vvsincosf(sine.baseAddress!, cosine.baseAddress!, theta.baseAddress!, &count)
                for i in hypotheses.indices {
                    ...
                    var inliers: Float = 0
                    for k in slice {
                        // peer → us in our frame, then into the peer's frame by −θ
                        // (the inverse of the rotation `ingestRemoteVIO` applies).
                        let dx = display.x - x[k]
                        let dz = display.z - z[k]
                        let inside: Bool
                        if hypot(dx, dz) < 0.1 {
                            inside = false
                        } else {
                            let peerX = dx * cosine[k] - dz * sine[k]
                            let peerZ = dz * cosine[k] + dx * sine[k]
                            let error = Self.wrap(atan2(peerX, peerZ) - psi)
                    ...
                    if inliers < 0.15 {
                        // A measurement the cloud cannot represent. Blame θ rather
                        // than position: positions are backed by range and our own
                        // bearing, θ by nothing else, and a bad θ should not throw
                        // away a good fix. Each re-seeded particle keeps its position
                        // and takes the θ that makes that position consistent with
                        // what the peer saw.
                        for k in slice where random.unit() < 0.3 {
                            let dx = x[k] - display.x
                            let dz = z[k] - display.z
                            guard hypot(dx, dz) >= 0.1 else { continue }
                            let phi = atan2(dx, dz)
                            theta[k] = Self.wrap(phi - psi + .pi + gauss(sigmaBearing))
                        }
                        normalize(weight, in: slice)
    ...
    /// Replace every particle's θ with a draw from N(theta, sigma).
    ///
    /// `FrameSolver` estimates θ from the whole window at once and reports a
    /// σ from the geometry, which is a better belief about θ than the cloud
    /// can form from one sample at a time. It is a summary of samples the
    /// cloud has already weighted on, so multiplying it in as a likelihood
    /// every second would count the same window over and over and collapse θ
    /// to a point. Replacing the draw keeps exactly the solver's spread and
    /// leaves positions alone: θ and position are separate state, and the
    /// positions are backed by range and bearing in their own right.
    func setThetaBelief(peerID: MCPeerID, theta: Float, sigma: Float) {
        guard let slot = slots[peerID], theta.isFinite, sigma.isFinite, sigma > 0 else { return }
        withCloud(at: slot) { cloud in
            cloud.withMutableBuffers { _, _, _, angles, _, _, _ in
                for k in angles.indices {
                    angles[k] = Self.wrap(theta + gauss(sigma))
                }
            }
        }
    }
    ...
    /// ψ that goes on the wire for the peer's `ingestPeerBearing`.
    ...
    /// Nearby Interaction's direction is in the portrait screen frame: +x to
    /// the right, +y toward the top of the phone, +z toward the user. ARKit's
    /// camera space is landscape: +x toward the Home button (portrait −y), +y
    /// toward the right edge (portrait +x), +z toward the user. Rotate into
    /// camera space first, then through the camera transform to world.
    ///
    /// The vector used to go straight into the camera transform, which is
    /// right only for a peer dead ahead of an upright phone. A flat phone put
    /// the peer 90° to the side, and a peer to the right of an upright phone
    /// projected to nearly vertical and vanished.
    private func niDirectionInWorld(_ direction: simd_float3) -> simd_float3? {
        ...
        let rotated = cameraTransform * SIMD4<Float>(-local.y, local.x, local.z, 0)
