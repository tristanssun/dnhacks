...
- `POST /video` with `X-Client-Id`, `X-Video-Name`, `X-Video-Duration` and an mp4 body: a scan recording from the phone (the app records `ARFrame.capturedImage` to HEVC while mapping, uploads at stop when collab is on, then deletes its local copy; with collab off the file stays next to the `.db`). Stored under `videos/` with a `.json` sidecar; names are sanitized to a plain file name. Optional `X-Address` (also accepted on `/join` and `/heartbeat`, or as JSON `address`) is the street address reverse-geocoded on the phone. The first address wins for that run and is stored on `clients.json`, `runs.json`, and the video/model sidecars. Run display names are `address · timestamp` (timestamp only until an address arrives). `GET /videos` lists recordings newest first and includes `address`, `title`, `run_name`; `GET /runs` is the run catalog. `GET /videos/<name>` streams with byte-range support (Safari needs it). The admin sidebar titles the current-run folder with that name and groups History recordings by run. A row click plays the file in an overlay. After reset, this run's recordings move into History.
- Video and model index (Gemini): after a video upload the server runs `server/video_summarize.py` in the background. It makes a 1 fps 640p proxy, sends that to Gemini, and writes `<name>.analysis.json` plus one line per completed task in `videos/tasks.jsonl`. When a room mesh is archived, the same worker indexes the model: it sends the texture atlas (if present) plus same-run walk tasks to Gemini and writes `<name>.analysis.json` plus one line per place in `models/index.jsonl`. Each row has `title`, `embed_text`, and a Gemini embedding. `GET /videos/<name>/analysis` is the player payload; `POST /videos/<name>/summarize` re-runs it; `GET /videos/tasks` is the video corpus. `GET /models/<name>/analysis` and `POST /models/<name>/index` are the model equivalents; `GET /models/index` is the model corpus. `GET /search?q=` (or `POST /search` with `{"q":"..."}`) ranks both corpora: vector search when a key is present, lexical otherwise. The History folder opens with a search box that hides models and recordings that do not match. Indexed tasks and places also count as a match. The player panel shows the video summary and seekable task chapters. Set `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) or put the key in `--data/gemini.key`. Optional: `GEMINI_MODEL` (default `gemini-3.6-flash`), `GEMINI_EMBED_MODEL` (default `gemini-embedding-001`). The LaunchAgent PATH must include Homebrew so `python3` and `ffmpeg` resolve.
- Room meshes are copied into `models/` when the room is reset or a new room starts (baked surface if one exists, otherwise the live mesh), newest 40 kept. A new archive is queued for Gemini indexing the same way a new video is. `GET /models` lists them (including `index_status` and `place_count`); `GET /models/<name>.ply` is the PLY and `GET /models/<stem>.jpg` is the atlas when the bake was textured. The admin sidebar shows them under History > Models. A row click loads that mesh in the 3D view; Live returns to the current room. Search hits on a model or place open that archive.
- `POST /join` with `X-Client-Id`: `mode: new` when nobody is active (the room is reset), `mode: join` otherwise. `must_download` is always false: a joiner starts a blank local session and receives other users' nodes through `/pull`; loading the room map locally would place it in the wrong frame. `must_wait_for_lock` tells the phone to point at the tag first. `lock_phones_required` is how many real tag detects lock the room.
- `POST /lock_phones` JSON `lock_phones_required` (1-16). Admin page control. Persists across reset. Lowering the count while phones are waiting can lock immediately.
...
- `POST /reset` wipes the room: map, bake, clients, and lock. The current mesh is archived under `models/` first. Phones must join and point at the tag again.
...
- `GET /demo` poll at 250 ms: `locked show_tag aligned lock_phones_required calibrated[] clients[]`; clients carry `x y z qx qy qz qw yaw` in G plus a ground-plane trail. `mesh_gen` changes with every live mesh; `mesh_baked bake_gen bake_max_node bake_textured` describe the assembled surface.
...
`POST /reset` archives the current mesh under `models/` then deletes the map and the bake. A `POST /join` into a room whose phones are all stale (45 s) starts a new room and drops both (after the same archive); a bake still running across that reset is discarded instead of resurrecting the old room.
