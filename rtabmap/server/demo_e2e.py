...
def main() -> int:
    ...
    try:
        ...
        check("lock-phones-input" in page and "/lock_phones" in page, "admin can set phones to start")
        ...
        check(int(demo.get("lock_phones_required") or 0) == 1, "default lock_phones_required 1")
        ...
        need2 = http_json("POST", base + "/lock_phones", {"lock_phones_required": 2})
        check(need2.get("ok") is True and int(need2.get("lock_phones_required") or 0) == 2, "POST /lock_phones 2")
        demo_need = http_json("GET", base + "/demo")
        check(int(demo_need.get("lock_phones_required") or 0) == 2, "demo reports lock_phones_required 2")
        join_need = http_json("POST", base + "/join", {}, "sim-need")
        check(int(join_need.get("lock_phones_required") or 0) == 2, "join reports lock_phones_required 2")
        bad_need = http_json("POST", base + "/lock_phones", {"lock_phones_required": 0})
        check(bad_need.get("ok") is False and bad_need.get("_http") == 400, "reject lock_phones_required 0")
        bad_need2 = http_json("POST", base + "/lock_phones", {"lock_phones_required": 99})
        check(bad_need2.get("ok") is False and bad_need2.get("_http") == 400, "reject lock_phones_required 99")
        http_json("POST", base + "/reset", {})
        http_json("POST", base + "/join", {}, "sim-a")
        wait_a = http_json("POST", base + "/calibrate", pose(tx=0.04, tz=0.42), "sim-a")
        check(wait_a.get("ok") is True and wait_a.get("locked") is False, "one phone waits when need=2")
        check(int(wait_a.get("calibrated_count") or 0) == 1, "waiting calibrated_count 1")
        wait_b = http_json("POST", base + "/calibrate", pose(tx=-0.03, tz=0.40), "sim-b")
        check(wait_b.get("ok") is True and wait_b.get("locked") is True, "second phone fulfills need=2")
        http_json("POST", base + "/lock_phones", {"lock_phones_required": 3})
        http_json("POST", base + "/reset", {})
        http_json("POST", base + "/join", {}, "sim-a")
        wait_three = http_json("POST", base + "/calibrate", pose(tx=0.04, tz=0.42), "sim-a")
        check(wait_three.get("ok") is True and wait_three.get("locked") is False, "one phone waits when need=3")
        drop = http_json("POST", base + "/lock_phones", {"lock_phones_required": 1})
        check(drop.get("ok") is True and drop.get("locked") is True, "lowering need locks immediately")
        demo_drop = http_json("GET", base + "/demo")
        check(demo_drop.get("locked") is True, "demo locked after lowering need")
        http_json("POST", base + "/lock_phones", {"lock_phones_required": 1})
        http_json("POST", base + "/reset", {})
        ...
        check('id="place-search"' in page and "Search places" in page and "runPlaceSearch" in page, "admin map has a Search places box")
        check("place-search-hits" in page and "searchNominatim" in page and "nominatim.openstreetmap.org/search?format=json&limit=5" in page, "place search queries OSM and scanned sites")
        check('id="scan-search"' in page and "filteredHistoryModels" in page, "admin History search filters folder rows")
